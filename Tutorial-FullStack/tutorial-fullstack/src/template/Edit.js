...
export const Edit = () => {
    ...
    const {
        ...
        defaultValues: async () => {
            const student = await axios.get(`http://localhost:8080/api/${id}`)
            return {...student.data, gender: student.data.gender.toString()}
        }
    })
    const handlePost = async (values) => {
        try {
            await axios.put("http://localhost:8080/api", values)
            navigate("/")
    ...
    return (
        <div>
            <h1 className="text-center text-uppercase text-secondary mb-3">Update Student</h1>
